...
Spawn multiple subagents after Step 1.2 is complete. Domain models are the shared contract — no subagent touches them after they are finalized.

### Phase 1 Parallel Tracks

After Step 1.2 domain models pass their test gate, split into two tracks:

| Track A — Engine | Track B — Test Infrastructure |
|------------------|-------------------------------|
| Step 1.3 Provider interface | Step 1.4 Fixture loader |
| Step 1.5 Mock provider | All 15 fixture JSON files (F1–F5 × diffs/lineage/expected) |
| Step 1.6 Diff parser | Unit test skeletons for all Track A modules |
| Step 1.7 Asset resolver | |
| Step 1.8 Traverser | |
| Step 1.9 Rules engine | |

**Sync point:** Both tracks must complete before Step 1.10 (report builder).
Steps 1.10, 1.11, 1.12 are sequential and cannot be parallelized.

### Phase 2 Parallel Tracks

| Subagent 1 | Subagent 2 | Subagent 3 |
|------------|------------|------------|
| OpenMetadataProvider REST | MCP client | JWT auth utility |

**Sync point:** All three unit tests pass → wire together in main.py → smoke test.

### Phase 3 Parallel Tracks

| Subagent 1 | Subagent 2 | Subagent 3 |
|------------|------------|------------|
| GitHub Action YAML | GitHubAdapter | CLI main.py |

**Sync point:** All three unit tests pass → integration test with mock PR.

### Coordination Rules

1. `src/domain/models.py` is frozen after Step 1.2. Any proposed change requires all active subagents to stop, review, re-test, and restart.
2. Subagents communicate only through the domain model interfaces and the fixture JSON files — never through shared mutable state.
3. If two subagents produce conflicting implementations, the one that satisfies more fixture tests wins.
